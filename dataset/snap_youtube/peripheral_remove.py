...
def community_remove(community_file, graph, removed_file):
        ...
        with open(community_file, 'r') as c:
                for line in c:
                        ...
                        removed_line = []
                        for v in line:
                                selected = graph.vs.select(id_eq = v)
                                if len(selected) > 0:
                                # if v in graph.vs:
                                        removed_line.append(selected[0].index)
...
edgelist = 'youtube_ungraph.txt'
community = 'youtube_community.txt'
...
print G.is_directed(), G.vcount(), G.ecount()
target_file = open("youtube_sampled_1", 'wb')
community_file = open('youtube_community_1', 'wb')
