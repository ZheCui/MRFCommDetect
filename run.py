...
result_file = open('_'.join(['result', dataset, str(sample_mode), str(theta), str(consider_cost), str(c)]), 'wb')
    ...
    while True:
        try:
            ...
            if algorithm.time_to_update_community() is True:
                communities = algorithm.update_community()
            if algorithm.get_current_cost() >= current_budget:
                communities = algorithm.update_community(False)
                communities = algorithm.update_community(True)
...
result_file.write('Entropy Avearge: \n')
result_file.write('[')
for budget in budget_list:
